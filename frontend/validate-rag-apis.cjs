...
const validateJsonApis = () => {
  ...
  const metadataFile = path.join(process.cwd(), 'public', 'api', 'metadata.json');
  ...
  apiFiles.forEach(fileName => {
    try {
      ...
      console.log(`✅ ${fileName} - Valid JSON with ${Object.keys(jsonData).length} root keys`);
      
      // Check for required fields based on file type
      if (fileName === 'all.json') {
        if (jsonData.website && jsonData.company && jsonData.products) {
          console.log(`   📋 Contains: website info, company details, product catalog`);
        }
      }
  ...
  try {
    ...
    if (robotsContent.includes('GPTBot') && robotsContent.includes('Claude-Web')) {
  ...
  try {
    ...
    const contentSitemapPath = path.join(process.cwd(), 'public', 'content-sitemap.xml');
